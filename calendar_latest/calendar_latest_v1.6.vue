<!--calendar 1.6版本 支持android和ios的vue模式，但不支持nvue模式。惯性动画和日历展开操作做了一些小优化，体验比之前更好。 修复了日历组件内没有东西时，向上滑动，然后出现日历错位问题。性能问题：组件在app的webview里，uniapp-App-Vue模式，下性能拉胯，性能卡顿目测是touchmove的问题。在原生chrome浏览器下性能不错。nvue下性能未测 -->
<template>
	<view class="calendar">
