<!--calendar 1.4版本 支持android和ios的vue模式，但不支持nvue模式。日历展开收起和滚动条上下滚动完成功能适配。性能方面得到优化。滑动松手后的动画编写完毕-->
<template>
	<view class="calendar">
		...
		methods: {
			...
			touchEnd(e) {
				...
				if (this.isTop == false && this.weekShow == true) {
					let move = e.changedTouches[0].pageY - this.startY;
					this.scrollAnimation(move);
				}
			...
			//滑动送手后，滚动条，减速动画
			scrollAnimation(move) {
				//move负值（手指从 手机底部 划向 手机顶部）
				//move正值（手指从手机顶部 划向 手机底部）

				let target = this.scrollTop - Math.floor(move * 4); //目标位置
				...
				// console.log('移动', target, move, this.scrollTop, this.scrollTopMax);

				//移动,  target 635.5999999999999 ,  move 66 ,  scrollTop 899.5999999999999 ,  scrollTopMax 2205.609375   看上面（正常）
				//移动,  target 1528 ,  move -291 ,  scrollTop 364 ,  scrollTopMax 2205.609375      看下面

				let step = null; //减速运动到目标位置时，移动时的每一步，移动多长的距离
				let scrollTop = null;
				this.setIntervalTimer = setInterval(() => {
					...
					if (this.scrollTop > target) {
						...
						if (target - this.scrollTop <= 2) {
							clearInterval(this.setIntervalTimer)
						}
					}

					// step = (target - this.scrollTop)/10;
					if (this.scrollTop > target) {
						step = (this.scrollTop - target) / 10;
						scrollTop = this.scrollTop - step;
					} else {
						step = (target - this.scrollTop) / 10;
						scrollTop = this.scrollTop + step;
					}

					// console.log(step, scrollTop)
