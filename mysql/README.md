...
### 基本“命令”
```
net start mysql //windows启动sql服务

net stop mysql //windows关闭sql服务

show databases; //显示所有数据库

show tables; //显示数据库里所有数据表

show columns from 数据表名; //显示数据表结构

show index from 数据表名; //显示数据表索引信息
...
举个栗子：mysqldump -uroot -p123456 map > D:\map.sql
...
//条件查询（可使用的运算符：关系运算符：=、<>、>、>=、<、<= ,逻辑运算符：NOT，AND，OR  ）
//查询入学成绩大于400且小于450的记录
select * from 成绩 where 入学成绩>=400 AND 入学成绩<=450
//查询以上的数据还可以用BETWEEN（指定区间）
