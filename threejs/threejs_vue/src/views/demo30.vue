...
//新增选中提示框
function addSelectPromptBox(size, center) {
    const { cube, lineSegments } = createTransparentCube(size.x, size.y, size.z, center.x, center.y, center.z)
    select_prompt_box = [cube, lineSegments]
}
//移除选中提示框
function removeSelectPromptBox() {
    for (let i = 0; i < select_prompt_box.length; i++) {
        removeMesh(select_prompt_box[i])
    }
}
...
renderer.domElement.addEventListener('click', (event) => {
    ...
    if (controlsMoveFlag) {
        if (intersects.length) {
            ...
            addSelectPromptBox(size, center) //新增选中提示框
        }
    }
}, false)
