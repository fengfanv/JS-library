...
//创建灯光
//没有光源的光。一般指环境光。环境光，你不知道光是从哪里发出的，你的前面，后面，左面，右面，上面，下面，一样亮。被环境光照到的物体，没有影子，因为环境光没有方向，周围一样亮。环境光会均匀的照亮场景中的所有物体。环境光，来自各个方向的光照。
//有光源的光。你知道光是从哪里发出的，被其 照到 的物体 有影子。其照射的光有方向。（平行光，点光源，聚光灯等等）
//创建一个“环境光”
...
meshFloor.rotation.x -= Math.PI / 2; //让“平面”以x轴为基准，旋转90deg。默认平面是竖着放的，所以这里才这样操作。
